...
import { Btn, Flex, MaterialSymbolIcon, pushNotice, useEventListener, useIsomorphicLayoutEffect, useScreen} from 'roku-ui';
      ...
      {user.data && (
        <Flex gap="1rem" align="center" direction={window.width <= 768 ? 'column': 'row'}>
          ...
          <Flex justify={(window?.width ?? 1920) < 768? 'center' : 'end'} style={{ width: '100%',maxWidth: '600px' , flexShrink: 10}} gap={'0.25rem'} >
            {boolList.map((d, i) => (
              <div key={i} style={{  height: '2rem',flexBasis: '0.3rem', backgroundColor: d ? 'hsl(var(--r-primary-2))' : 'hsl(var(--r-default-2))' }}></div>
            ))}
          </Flex>
        </Flex>
      )}
